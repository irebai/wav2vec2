...
@dataclass
class DataCollatorS2SWithPadding:
    """
    Data collator that will dynamically pad the inputs received.
    Args:
        processor (:class:`~transformers.Wav2Vec2Processor`)
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        label_features = [{"input_ids": feature["labels"]} for feature in features]
        label_bos_features = [{"input_ids": [self.processor.tokenizer.bos_token_id] + feature["labels"][:-1]} for feature in features]
        label_eos_features = [{"input_ids": feature["labels"][:-1] + [self.processor.tokenizer.eos_token_id]} for feature in features]
        ...
        with self.processor.as_target_processor():
            ...
            labels_bos_batch = self.processor.pad(
                label_bos_features,
                padding=self.padding,
                max_length=self.max_length_labels,
                pad_to_multiple_of=self.pad_to_multiple_of_labels,
            ...
            labels_eos_batch = self.processor.pad(
                label_eos_features,
                padding=self.padding,
                max_length=self.max_length_labels,
                pad_to_multiple_of=self.pad_to_multiple_of_labels,
        ...
        # replace padding with -100 to ignore loss correctly
        labels = labels_batch["input_ids"].masked_fill(labels_batch.attention_mask.ne(1), -100)
        labels_bos = labels_bos_batch["input_ids"].masked_fill(labels_batch.attention_mask.ne(1), -100)
        labels_eos = labels_eos_batch["input_ids"].masked_fill(labels_batch.attention_mask.ne(1), -100)

        batch["labels"] = labels
        batch["labels_bos"] = labels_bos
        batch["labels_eos"] = labels_eos
